...
import { findStaffResetToken, markStaffResetTokenUsed } from "@/lib/passwordResetTokens";
...
export async function POST(request) {
  try {
    ...
    const resetToken = await findStaffResetToken(hashResetToken(token));
    if (!resetToken) {
    ...
    await updateStaffPassword(resetToken.user_id, passwordHash);
    await markStaffResetTokenUsed(resetToken.id);
