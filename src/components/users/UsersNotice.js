...
const COPY = {
  401: { title: "You're signed out", message: "Sign in again to manage users." },
  403: { title: "Access denied", message: "Your role doesn't allow this. Ask a Super Admin for access." },
  404: { title: "User not found", message: "This user doesn't exist or was deleted." },
  503: {
    title: "Users isn't set up yet",
    message: "Run `npm run db:migrate:staff-users` and `npm run db:migrate:admin-roles`.",
  },
};

// Server-safe card for "can't show this screen" states: no permission, the
// tables aren't migrated yet, or a user that doesn't exist.
export default function UsersNotice({ status = 500, message, backHref }) {
  const copy = COPY[status] || { title: "Couldn't load users", message: "Something went wrong. Try again." };
