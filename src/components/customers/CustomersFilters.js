...
import { GROUP_LABELS, TYPE_LABELS } from "./customerHelpers";

const GROUP_OPTIONS = ["All groups", ...Object.keys(GROUP_LABELS)];
...
export default function CustomersFilters({
  ...
  return (
    <section className="bg-white dark:bg-darksurface border border-slate-200 dark:border-white/5 rounded-2xl shadow-card p-4 sm:p-5">
        ...
        <div className="relative w-full md:w-44 shrink-0">
          ...
          >
            {GROUP_OPTIONS.map((opt) => (
              <option key={opt} value={opt === "All groups" ? "" : opt}>
                {opt === "All groups" ? opt : GROUP_LABELS[opt]}
              </option>
            ))}
          </select>
      ...
      <p className="text-xs text-slate-400 mt-3">{resultCount} customer{resultCount === 1 ? "" : "s"} found</p>
    </section>
  );
}
